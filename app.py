...
#db_str = "mysql+mysqldb://dspuser:dsp@2021!@202.144.157.56/dsp_db" 
...
import pymysql
pg_engine = "user='{}' password='{}' host='{}' dbname='{}'".format('dspuser','dsp@2021!', '202.144.157.56', 'dsp_db')
db_connection = pymysql.connect(host = '202.144.157.56', user = 'dspuser', 
                                password = 'dsp@2021!', db = 'dsp_db')
...
result = pd.read_sql_query(sql_query, db_connection)
...
def on_button_click(n):
    if n is None:
        ...
        g = df[['gender']].value_counts().to_frame(name = 'number')
        g.reset_index(inplace = True)
        fig = px.bar(g, x= 'gender', y = 'number', color = 'gender')
    return fig
